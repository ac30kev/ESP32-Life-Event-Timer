/*
   ...
   REV 0.7.5 - works but calculation wildly out
   REV 0.7.6 - got it working!!!
   REV 0.7.7 - trying to sort out discrepency if target < 1 year
   REV 0.8.0 - added lots of new features (see notes)
   REV 0.8.1 - fixed check all wifi and see if a match
   REV 0.8.2 - highlighted pre-chosen options when target and wifi screens re-entered.
*/
...
char formattedTargetTime[10];

int selectedWiFiIndex = -1;
int selectedTargetIndex = -1;// Initialize with -1 to indicate no option selected
...
void calculateTimeDifference(unsigned long currentTime, const char* formattedTargetDate, const char* formattedTargetTime);
...
void displayInputScreen(InputScreenType screenType);
void handleInputScreenTouchEvent(InputScreenType screenType);
...
// Define a separate array to store target dates as String objects
String targetDatesArray[sizeof(targetOptions) / sizeof(targetOptions[0])];
// Define separate arrays to store SSID names and passwords as String objects
String ssidArray[sizeof(options) / sizeof(options[0])];
String passwordArray[sizeof(options) / sizeof(options[0])];
            ...
            for (int i = 0; i < (currentScreenState == WIFI_SCREEN ? sizeof(options) / sizeof(options[0]) : sizeof(targetOptions) / sizeof(targetOptions[0])); i++) {
                ...
                if (p.x >= 700 && p.x <= 3200 && p.y >= buttonY && p.y <= buttonY + 350) {
                    ...
                    if (currentScreenState == WIFI_SCREEN) {
                        ...
                        drawTargetOptions(highlightedOption);  // Call drawTargetOptions with a single argument
                    }
                    delay(200);  // Debounce
...
bool checkWiFiCredentials() {
  ...
  Serial.println("Cannot connect to the selected WiFi network");
  return false;
}
...
void setup() {
  ...
  preferences.begin("countdown_timer", false);
 selectedWiFiIndex  = findMatchingWiFiOption(selectedWiFiOption.name, selectedWiFiOption.password);
  // Read target dates from Preferences
  for (int i = 0; i < sizeof(targetOptions) / sizeof(targetOptions[0]); i++) {
    String targetKey = "target_" + String(i);
    if (preferences.isKey(targetKey.c_str())) {
      targetDatesArray[i] = preferences.getString(targetKey.c_str(), targetOptions[i].target);
    } else {
      targetDatesArray[i] = targetOptions[i].target;
    }
  }
  ...
  for (int i = 0; i < sizeof(options) / sizeof(options[0]); i++) {
    ...
    String passwordKey = "password_" + String(i);
    if (preferences.isKey(ssidKey.c_str()) && preferences.isKey(passwordKey.c_str())) {
      ssidArray[i] = preferences.getString(ssidKey.c_str(), options[i].name);
      passwordArray[i] = preferences.getString(passwordKey.c_str(), options[i].password);
    } else {
      ssidArray[i] = options[i].name;
      passwordArray[i] = options[i].password;
    }
  }
// Find the index of the currently selected target date

  for (int i = 0; i < sizeof(targetOptions) / sizeof(targetOptions[0]); i++) {
    if (targetDatesArray[i] == selectedTargetDate) {
  ...
  if (checkWiFiCredentials()) {
    ...
    // WiFi connection failed, display WiFi screen or take appropriate action
...
void showWelcomeScreen() {
  ...
  tft.setTextDatum(MC_DATUM);
  tft.drawString("Welcome to", tft.width() / 2, 50);
  tft.setTextSize(2);
  tft.drawString("The Timer", tft.width() / 2, 100);
  tft.setTextSize(1);
  tft.drawString("Checking stored WiFi details", tft.width() / 2, 150);
  tft.drawString("to see if any match local.", tft.width() / 2, 180);
  tft.setTextFont(1);
  tft.setTextColor(TFT_SKYBLUE);
  tft.drawString("(C) ac30kev - 2024 --- ", 100, 220);
  tft.drawString(ver,185,220);
  tft.setTextFont(4);
  delay(3000);
}

void handleWelcomeTouchEvent() {
  if (ts.tirqTouched() && ts.touched()) {
    TS_Point p = ts.getPoint();
    if (p.x >= 110 && p.x <= 110 + 100 && p.y >= 200 && p.y <= 200 + 27) {
      tft.fillScreen(TFT_BLACK);
      displayWiFiScreen();
    }
  }
...
void renderMainScreen(unsigned long epochTime) {
  ...
  calculateTimeDifference(epochTime, formattedTargetDate, formattedTargetTime);
...
void displayWiFiScreen() {
  ...
  // Find the index of the currently selected WiFi option
  int selectedIndex = -1;
  for (int i = 0; i < sizeof(options) / sizeof(options[0]); i++) {
    if (options[i].name == selectedWiFiOption.name && options[i].password == selectedWiFiOption.password) {
      selectedIndex = i;
      break;
    }
  }

  drawWiFiOptions(selectedIndex);
  drawConfirmButton(170, 200, 100, 27, TFT_GREEN, TFT_BLACK, 5, "Confirm");
...
void displayTargetScreen() {
    ...
    displayBackButtonData();  // Display back button symbol
    drawTargetOptionsWithTime(selectedTargetIndex, formattedTargetTime);  // Call drawTargetOptionsWithTime
    drawConfirmButton(170, 200, 100, 27, TFT_GREEN, TFT_BLACK, 5, "Confirm");
    drawButton(50, 200, 100, 27, TFT_SKYBLUE, TFT_BLACK, 5, "New Date");
    currentScreenState = TARGET_SCREEN;
}

void handleTargetScreenTouchEvent() {
  if (ts.tirqTouched() && ts.touched()) {
    TS_Point p = ts.getPoint();
    if (p.x >= 50 && p.x <= 50 + 100 && p.y >= 200 && p.y <= 200 + 27) {
      // New Date button pressed
      displayInputScreen(INPUT_SCREEN_TARGET);
    }
  }
}
...
void displayInputScreen(InputScreenType screenType) {
  ...
  String title;
  if (screenType == INPUT_SCREEN_WIFI) {
    title = "Enter WiFi Credentials";
  } else {
    title = "Enter Target Date";
  }
  tft.fillScreen(TFT_BLACK);
  tft.drawString(title, tft.width() / 2, 14);
  displayBackButtonData();  // Display back button symbol

  // Add input fields and buttons for entering data
  // ...
}
...
void handleInputScreenTouchEvent(InputScreenType screenType) {
  if (ts.tirqTouched() && ts.touched()) {
    ...
    // Handle touch events for input fields and buttons
    // ...

    // Save data to the appropriate array
    if (screenType == INPUT_SCREEN_WIFI) {
      // Save WiFi credentials to options array
      saveWiFiCredentials();
    } else {
      // Save target date to targetOptions array
      saveTargetDates();
    }
  }
}
...
void drawWiFiButton(int16_t x, int16_t y, int16_t w, int16_t h, uint8_t radius, const char* text, uint8_t fontSize = 1, uint8_t font = 1, bool highlighted = false) {
  if (highlighted) {
    ...
    tft.fillRoundRect(x, y, w, h, radius, TFT_LIGHTGREY);
  }
  tft.drawRoundRect(x - 2, y - 2, w + 4, h + 4, radius + 2, TFT_DARKGREY);
  tft.setTextColor(TFT_BLACK, highlighted ? TFT_YELLOW : TFT_LIGHTGREY);
  tft.setTextDatum(MC_DATUM);
...
void drawWiFiOptions(int highlightedIndex) {
  ...
  for (int i = 0; i < sizeof(options) / sizeof(options[0]); i++) {
    String displayText = ssidArray[i] + " (" + String(options[i].nickname) + ")";
...
void drawTargetOptions(int highlightedIndex) {
    tft.fillRect(8, 30, 240, 150, TFT_BLACK);  // Clear previous options
    for (int i = 0; i < sizeof(targetOptions) / sizeof(targetOptions[0]); i++) {
        String displayText = targetDatesArray[i];
        drawWiFiButton(50, 40 + i * 30, 220, 20, 5, displayText.c_str(), 1, 2, i == highlightedIndex);
    }
    tft.setTextFont(4);
}

void drawTargetOptionsWithTime(int highlightedIndex, const char* formattedTargetTime) {
    tft.fillRect(8, 30, 240, 150, TFT_BLACK);  // Clear previous options
    for (int i = 0; i < sizeof(targetOptions) / sizeof(targetOptions[0]); i++) {
        String displayText = targetDatesArray[i] + " - " + String(formattedTargetTime);
        drawWiFiButton(50, 40 + i * 30, 220, 20, 5, displayText.c_str(), 1, 2, i == highlightedIndex);
    }
    tft.setTextFont(4);
}
...
void displayCountdownOnTFT() {
    // Get the target date details from the selectedTargetDate variable
    calculateTimeDifference(timeClient.getEpochTime(), selectedTargetDate.c_str(), targetOptions[highlightedOption].trgtTime);
    ...
    // Print the formatted target date and time
    char formattedTargetDateTime[30];
    sprintf(formattedTargetDateTime, "%s - %s", targetOptions[highlightedOption].trgtTime, selectedTargetDate.c_str());
    int textWidth = tft.textWidth(formattedTargetDateTime);
    int textX = (tft.width() - textWidth) / 2;
    int textY = 200;

    int rectX = (tft.width() - textWidth) / 2 - 10;
    int rectWidth = textWidth + 20;
    tft.drawRoundRect(rectX, textY - 4, rectWidth, 32, 5, TFT_MAGENTA);
    tft.setCursor(textX, textY);
    tft.println(formattedTargetDateTime);
}

void calculateTimeDifference(unsigned long currentTime, const char* formattedTargetDate, const char* formattedTargetTime) {
    // Define and initialize a struct tm variable for the target time
    struct tm targetTm = {};
    if (strptime(formattedTargetDate, "%d/%m/%Y", &targetTm) == NULL) {
        // Serial.print("Error parsing target date: ");
        //Serial.println(formattedTargetDate);
        return;  // Exit the function if parsing fails
    }

    // Parse target time (H:M)
    int hours, minutes;
    sscanf(formattedTargetTime, "%d:%d", &hours, &minutes);
    targetTm.tm_hour = hours;
    targetTm.tm_min = minutes;
    targetTm.tm_sec = 0;  // Set seconds to 0

    // Convert targetTm struct to a time_t value (seconds since epoch)
    time_t targetDateTime = mktime(&targetTm);
    if (targetDateTime == -1) {
        //Serial.println("Error converting target time to timestamp");
    ...
    // Convert time difference to struct tm
    struct tm timeDifferenceTm = *gmtime(&timeDifferenceSeconds);

    // Extract years, months, days, hours, minutes, and seconds
    countdownValues[0] = timeDifferenceTm.tm_year-70;  // Years since 1900
    countdownValues[1] = timeDifferenceTm.tm_mon;   // Months (0-11)
    countdownValues[2] = timeDifferenceTm.tm_mday - 1;  // Days (1-31)
    countdownValues[3] = timeDifferenceTm.tm_hour - 1;  // Hours (0-23)
    countdownValues[4] = timeDifferenceTm.tm_min;   // Minutes (0-59)
    countdownValues[5] = timeDifferenceTm.tm_sec;   // Seconds (0-61)
}
...
void handleTouchEvent() {
  ...
  if (p.z > 0) {
    ...
    else if ((currentScreenState == WIFI_SCREEN || currentScreenState == TARGET_SCREEN) && mappedX >= 170 && mappedX <= 170 + 100 && mappedY >= 200 && mappedY <= 200 + 27) {
      Serial.println("Confirm button pressed");
      if (currentScreenState == WIFI_SCREEN) {
        selectedWiFiOption = options[highlightedOption]; // Store the selected WiFi option
        ...
        if (checkWiFiCredentials()) {
          ...
          // Handle the case when the connection to the selected WiFi fails
          tft.fillScreen(TFT_BLACK);
          tft.setTextColor(TFT_RED, TFT_BLACK);
          tft.setTextDatum(MC_DATUM);
          tft.drawString("Cannot connect to the selected WiFi", tft.width() / 2, tft.height() / 2);
          delay(2000); // Wait for 2 seconds before returning to the WiFi screen
          displayWiFiScreen();
        }
       } else if (currentScreenState == TARGET_SCREEN) {
        selectedTargetIndex = highlightedOption;  // Store the index of the selected target option
...
int findMatchingWiFiOption(const char* ssid, const char* password) {
  for (int i = 0; i < sizeof(options) / sizeof(options[0]); i++) {
    if (strcmp(options[i].name, ssid) == 0 && strcmp(options[i].password, password) == 0) {
      return i;  // Return the index of the matching option
    }
  }
...
void loop() {
  ...
  switch (currentScreenState) {
    ...
    case WIFI_SCREEN:
      ...
      handleWiFiScreenTouchEvent();
      break;
    case TARGET_SCREEN:
      // Render the target screen
      handleTargetScreenTouchEvent();
      break;
  }
  handleTouchEvent();
  handleWiFiTouchEvent();
  handleTargetTouchEvent();
  handleWelcomeTouchEvent();
  delay(100);
}

void saveTargetDates() {
  // Save target dates to Preferences
  for (int i = 0; i < sizeof(targetOptions) / sizeof(targetOptions[0]); i++) {
    String targetKey = "target_" + String(i);
    preferences.putString(targetKey.c_str(), targetDatesArray[i]);
  }
}
...
void saveWiFiCredentials() {
  ...
  for (int i = 0; i < sizeof(options) / sizeof(options[0]); i++) {
    String ssidKey = "ssid_" + String(i);
    String passwordKey = "password_" + String(i);
    preferences.putString(ssidKey.c_str(), ssidArray[i]);
    preferences.putString(passwordKey.c_str(), passwordArray[i]);
  }
}
  ...
  if (ts.tirqTouched() && ts.touched()) {
    ...
    if (currentScreenState == WIFI_SCREEN) {
      for (int i = 0; i < sizeof(options) / sizeof(options[0]); i++) {
        ...
        if (p.x >= 700 && p.x <= 3200 && p.y >= buttonY && p.y <= buttonY + 350) {
          highlightedOption = i;               // Store the touched option index
          drawWiFiOptions(highlightedOption);  // Redraw options with the highlighted option
          delay(200);                          // Debounce
          break;                               // Exit loop once a button is pressed
        }
      }
    ...
    if (ts.tirqTouched() && ts.touched()) {
        ...
        if (currentScreenState == TARGET_SCREEN) {
            for (int i = 0; i < sizeof(targetOptions) / sizeof(targetOptions[0]); i++) {
                ...
                if (p.x >= 700 && p.x <= 3200 && p.y >= buttonY && p.y <= buttonY + 350) {
                    highlightedOption = i;  // Store the touched option index
                    selectedTargetDate = targetOptions[highlightedOption].target;  // Update the selectedTargetDate
                    drawTargetOptions(highlightedOption);  // Call drawTargetOptions with a single argument
                    delay(200);  // Debounce
                    break;       // Exit loop once a button is pressed
                }
...
void handleWiFiScreenTouchEvent() {
  if (ts.tirqTouched() && ts.touched()) {
    TS_Point p = ts.getPoint();

    if (p.x >= 50 && p.x <= 50 + 100 && p.y >= 150 && p.y <= 150 + 27) {
      // Try Again button pressed
      displayWiFiScreen();
    } else if (p.x >= 170 && p.x <= 170 + 100 && p.y >= 150 && p.y <= 150 + 27) {
      // New Data button pressed
      displayInputScreen(INPUT_SCREEN_WIFI);
    }
  }
}
