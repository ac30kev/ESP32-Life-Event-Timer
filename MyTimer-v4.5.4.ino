...
uint16_t bgColor = TFT_BLACK;  // black
uint16_t timeColor = 0x0d18;   // light cyan
uint16_t boxColor = 0x0082;// was 0x01e8
uint16_t countdownColor = boxColor;
uint16_t dateColor = 0x0390;
uint16_t targetColor = 0xbde0;//gold
uint16_t welcomeColor = dateColor;

TFT_eSPI tft = TFT_eSPI();
Preferences preferences;

int invert = 1;  //0=std, 1=inverted screen colours
...
const unsigned long REMAINING_TIME_HIGHLIGHT_DURATION = 10000;  // 10 seconds
...
void handleTouch(uint16_t x, uint16_t y) {
  ...
  if (currentTime - lastTouchTime < TOUCH_DEBOUNCE_TIME || (abs(x - lastX) < 10 && abs(y - lastY) < 10)) {
    return;
  }
...
void setBacklight(int intensity) {
  intensity = constrain(intensity, minBacklight, userSetBacklight);
...
void adjustBacklight() {
  ...
  // Constants for LDR values
  const int LDR_MIN = 0;    // LDR value in bright conditions
  const int LDR_MAX = 900;  // LDR value in dark conditions (adjust if needed)

  // Calculate the range based on userSetBacklight
  int brightnessRange = userSetBacklight - minBacklight;

  // Map LDR value to backlight intensity within the user-set range
  newIntensity = map(ldrValue, LDR_MIN, LDR_MAX, userSetBacklight, userSetBacklight - brightnessRange);
  newIntensity = constrain(newIntensity, minBacklight, userSetBacklight);

  // Apply smoothing
  backlightIntensity = (backlightIntensity * 3 + newIntensity) / 4;
  setBacklight(backlightIntensity);

  // Debug print (you can remove this later)
  //Serial.printf("LDR: %d, New Intensity: %d, User Set Max: %d\n", ldrValue, newIntensity, userSetBacklight);
}
...
void drawSlider(int value) {
  int sliderPosition = map(value, minBacklight, 255, 0, SLIDER_WIDTH);

  // Clear the entire slider area
  tft.fillRect(SLIDER_X - KNOB_RADIUS, SLIDER_Y - KNOB_RADIUS, 
               SLIDER_WIDTH + KNOB_RADIUS * 2, SLIDER_HEIGHT + KNOB_RADIUS * 2, bgColor);
  ...
  tft.fillRect(0, SLIDER_Y - 60, tft.width(), 40, bgColor);
...
void toggleAlarm(int index) {
  ...
  // Redraw only the affected toggle switch
  drawToggleSwitch(220, 50 + index * 40 + 5, alarms[index].enabled);
  
  // Add a small delay to ensure the new state is visible
  delay(100);
}
...
void testLDR() {
  int ldrValue = analogRead(LDR_PIN);
  Serial.printf("LDR Test - Raw Value: %d\n", ldrValue);
...
void drawToggleSwitch(int x, int y, bool state) {
  ...
  // Draw background
  tft.fillRoundRect(x, y, width, height, height / 2, state ? TFT_GREEN : TFT_DARKGREY);
  tft.drawRoundRect(x, y, width, height, height / 2, TFT_WHITE);

  // Draw knob
  int knobX = state ? x + width - knobSize - 2 : x + 2;
  tft.fillCircle(knobX + knobSize / 2, y + height / 2, knobSize / 2, TFT_WHITE);

  // Draw state text
  tft.setTextColor(state ? TFT_DARKGREEN : TFT_LIGHTGREY);
  tft.setTextDatum(MC_DATUM);
  tft.setTextFont(2);
  tft.drawString(state ? "ON" : "OFF", x + width / 2, y + height / 2);
}
...
void setup() {
  ...
  copyright = 1;
  displayMessage("Welcome to The Timer", "(C) ac30kev 2024. Rev 4.5.4 ", welcomeColor, 2000);
  delay(2000);
  ...
  Serial.println("Countdown Timer v4.5.4");
}
...
void loop() {
  ...
  // Adjust backlight every minute
  static unsigned long lastBacklightAdjust = 0;
  if (currentMillis - lastBacklightAdjust >=60000) {
    lastBacklightAdjust = currentMillis;
    adjustBacklight();
  }
