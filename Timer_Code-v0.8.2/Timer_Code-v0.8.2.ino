...
void showWelcomeScreen() {
  ...
  tft.drawString("(C) ac30kev - 2024 --- V0.8.1)", 100, 220);
  tft.setTextFont(4);
  delay(3000);
}
...
void displayTargetScreen() {
    ...
    displayBackButtonData();  // Display back button symbol
    drawTargetOptions(selectedTargetIndex);  // Highlight the previously selected target option
    drawConfirmButton(170, 200, 100, 27, TFT_GREEN, TFT_BLACK, 5, "Confirm");
...
void displayCountdownOnTFT() {
  ...
  calculateTimeDifference(timeClient.getEpochTime(), selectedTargetDate.c_str(), formattedTargetTime);
  ...
  char formattedTargetDate[20];
  // Extract hours and minutes from formattedTargetTime
  int hours, minutes;
  sscanf(formattedTargetTime, "%d:%d", &hours, &minutes);
  sprintf(formattedTargetDate, "%02d:%02d - %s", 0, 0, selectedTargetDate.c_str(), formattedTargetTime);  // Assuming the time is 00:00
  int textWidth = tft.textWidth(formattedTargetDate);
  ...
  tft.println(formattedTargetDate);
}
...
void calculateTimeDifference(unsigned long currentTime, const char* formattedTargetDate, const char* formattedTargetTime) {
  ...
  sscanf(formattedTargetTime, "%d:%d", &targetTm.tm_hour, &targetTm.tm_min);
  ...
  countdownValues[0] = timeDifferenceTm.tm_year - 70;  // Years since 1900, adjusted for the offset
